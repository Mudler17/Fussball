...
export const generatePhilosophicalAnalysis = async (
  ...
  try {
    ...
    return `⚠️ **Fehler bei der Analyse**\n\n\`\`\`\n${msg}\n\`\`\`\n\nPrüfe:\n- Ist \`GEMINI_API_KEY\` in Coolify als Environment-Variable gesetzt?\n- Ruf \`/api/debug\` direkt im Browser auf, um die Gemini-Verbindung zu testen.\n- Schau in die Coolify-Logs des Services.`;
  }
};
...
export const generateMatchCommentary = async (
  ...
  try {
    ...
    const msg = err instanceof Error ? err.message : String(err);
    console.error('Kommentar-Fehler:', msg);
    return null;
  }
};
...
export const generateLockerRoomTalk = async (
  ...
  try {
    ...
    const msg = err instanceof Error ? err.message : String(err);
    console.error('Kabinen-Fehler:', msg);
    return `⚠️ **Fehler:**\n\n\`\`\`\n${msg}\n\`\`\``;
  }
};
